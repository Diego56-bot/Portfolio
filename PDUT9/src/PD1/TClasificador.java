...
public class TClasificador {
     ...
     */
    public int[] clasificar(int[] datosaClasificar, int metodoClasificacion) {
        switch (metodoClasificacion) {
            case METODO_CLASIFICACION_INSERCION:
                return ordenarPorInsercion(datosaClasificar);
            case METODO_CLASIFICACION_SHELL:
                return ordenarPorShell(datosaClasificar);
            case METODO_CLASIFICACION_BURBUJA:
                return ordenarPorBurbuja(datosaClasificar);
            case METODO_CLASIFICACION_QUICKSORT:
                return ordenarPorQuickSort(datosaClasificar);
            default:
                System.err.println("Usted debería irse");
                break;
        }
        return datosaClasificar;
    }
    ...
    private int[] ordenarPorShell(int[] datosParaClasificar) {
        int j, inc;
        int[] incrementos = new int[] { 1244, 456, 43, 23, 5, 2 };
    ...
    public static void main(String args[]) {
        ...
        System.out.println();
        int[] Ascendente = clasif.clasificar(vectorAscendente,
                METODO_CLASIFICACION_QUICKSORT);
        for (int i = 0; i < Ascendente.length; i++) {
            System.out.print(Ascendente[i] + " ");
        }
        System.out.println();
        int[] Descendente = clasif.clasificar(vectorDescendente,
